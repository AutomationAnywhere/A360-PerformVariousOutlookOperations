...
Code updated by:
    Sikha P | Partner Solution Engineer | Partner Solution Desk(PSD)
 */
...
namespace OutlookWrapper
{
    public class OutlookWrapper
    {
        ...
        /************************************/
        /*Below 2 Functions are newly added and it can help the DLL to work seamlessly in A360 platform
       
        1 . SelectProfileRetryWhenMarshalError() -> This function has an additional exception handler to handle the exception that comes in Marshal.GetActiveObject()
        2 . LaunchOutlook(int DelayAfterLaunch) -> This function will add delay as configured in the input parameter.The delay is provided in secs.

        Below steps helped for Select profile isssue -
        Upload the new DLL(refer attachments) in CR and modify the bot to use the new DLL.
        Replace the function "LaunchOutlook" in the "LaunchOutlook" bot inside OutlookV2 with "LaunchOutlook(int DelayAfterLaunch)" function, provide input value as 30 sec and check if that fixes the issue.Then try reducing the delay number as per requirement. 
        Replace the function "SelectProfile" in the "SelectProfile" bot inside OutlookV2 with "SelectProfileRetryWhenMarshalError" function and it should fix the issue.
        
        KB article for the reference : https://apeople.automationanywhere.com/s/article/Issue-with-OutlookV2-Metabot-function---Select-Profile
         */
        /************************************/

        public string SelectProfileRetryWhenMarshalError()
        {
            string empty1 = string.Empty;
            string empty2 = string.Empty;
            try
            {
                if (((IEnumerable<Process>)Process.GetProcessesByName("OUTLOOK")).Count<Process>() > 0)
                {
                    try
                    {
                        this.application = Marshal.GetActiveObject("Outlook.Application") as Microsoft.Office.Interop.Outlook.Application;
                    ...
                    {
                        this.application = (Microsoft.Office.Interop.Outlook.Application)Activator.CreateInstance(Marshal.GetTypeFromCLSID(new Guid("0006F03A-0000-0000-C000-000000000046")));
                        // ISSUE: reference to a compiler-generated method
                        this.nameSpace = this.application.GetNamespace("MAPI");
                        this.isOutlookVisible = false;
                    }
                }
                else
                {
                ...
                // ISSUE: reference to a compiler-generated method
                this.nameSpace.Logon((object)empty1, (object)empty2, (object)false, (object)true);
                this.profileNameSaved = empty1;
                this.profilePasswordSaved = empty2;
                return "Success";
            }
            catch (Exception ex)
            {
                this.ReleaseComObject((object)this.application);
                this.ReleaseComObject((object)this.nameSpace);
                this.ReleaseComObject((object)this.account);
                this.ReleaseComObject((object)this.folder);
                this.ReleaseComObject((object)this.mail);
                return "[ERROR]:" + ex.ToString();
            }
        }

        public string SelectProfileRetry20Times()
        {
            string empty1 = string.Empty;
            string empty2 = string.Empty;
            bool flag = false;
        ...
        public string LaunchOutlook(int DelayAfterLaunch)
        {
            try
            {
                if (((IEnumerable<Process>)Process.GetProcessesByName("OUTLOOK")).Count<Process>() == 0)
                {
                    ...
                    process.StartInfo = new ProcessStartInfo("OUTLOOK.EXE");
                    process.Start();
                    Thread.Sleep(DelayAfterLaunch * 1000);
                    process.WaitForInputIdle();
                    OutlookWrapper.ShowWindow(process.MainWindowHandle, 6);

                    //OutlookWrapper.ShowWindow(process.MainWindowHandle, 6);
                    Thread.Sleep(3000);
                    OutlookWrapper.ShowWindow(process.MainWindowHandle, 3);
                    Thread.Sleep(2000);
                    this.isOutlookVisible = true;
                }
                return "Success";
